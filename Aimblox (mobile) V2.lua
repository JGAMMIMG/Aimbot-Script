...
-- Função para criar um indicador de alvo
local function CreateTargetIndicator(parent)
    local indicator = Instance.new("BillboardGui", parent)
    indicator.Size = UDim2.new(0, 100, 0, 100) -- Tamanho do indicador
    indicator.AlwaysOnTop = true
    indicator.Adornee = parent
    
    local frame = Instance.new("Frame", indicator)
    frame.Size = UDim2.new(1, 0, 1, 0)
    frame.BackgroundColor3 = Color3.fromRGB(255, 0, 0) -- Cor do indicador (vermelho)
...
local function AimAtTarget()
    if target then
        ...
        local humanoidRootPart = FindHumanoidRootPart(target.Parent)
        if humanoidRootPart then
            CreateTargetIndicator(humanoidRootPart) -- Criar um indicador ao redor do alvo
        end
    end
end
...
local function ToggleCameraMode()
    ...
    if player.CameraMode == Enum.CameraMode.LockFirstPerson then
        ...
        player.CameraMode é Enum.CameraMode.LockFirstPerson
    end
end
...
local function CreateMobileButtons()
    ...
    aimButton.Size é UDim2.new(0.1, 0, 0.1, 0)
    aimButton.Position é UDim2.new(0.85, 0, 0.366, 0)
    aimButton.Text é "Mirar"
    aimButton.BackgroundColor3 é Color3.fromRGB(50, 200, 50)
